    ...
    <div class="py-6">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center space-x-2 text-gray-400 text-sm">
          ...
          <a href="{{ URL::to('categories/' . $item->categories[0]->name) }}" class="hover:underline hover:text-gray-600">{{ $item->categories[0]->name }}</a>
          <span>
            <svg class="h-5 w-5 leading-none text-gray-300" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
            </svg>
      ...
      <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8 mt-6">
        <div class="flex flex-col md:flex-row -mx-4">
          <div class="md:flex-1 px-0 md:px-4">
            <div x-data="{ image: 1 }" x-cloak>
              <div class="rounded-lg mb-4">
                ...
                @for ($i = 1; $i <= 4; $i++)
                    <div x-show="image === {{ $i }}" class="rounded-lg mb-4 flex items-center justify-center">
              ...
              <div class="flex -mx-2 mb-4 px-4 md:px-0">
                <template x-for="i in 4">
                  <div class="flex-1 px-2">
                    <button x-on:click="image = i" :class="{ 'ring-2 ring-indigo-300 ring-inset': image === i }" class="focus:outline-none w-full rounded-lg h-20 md:h-30 bg-gray-100 flex items-center justify-center">
          ...
          <div class="md:flex-1 px-4">
            <h2 class="mb-2 leading-tight tracking-tight font-bold text-gray-100 text-2xl md:text-3xl">{{ $item->name }}</h2>
            ...
            <div class="flex py-4 space-x-4">
              ...
              <button type="button" class="h-14 px-6 py-2 font-semibold rounded-xl bg-gray-200 text-gray-700 hover:bg-indigo-500 hover:text-gray-100">
                Add to Cart
              </button>
            </div>
          </div>
